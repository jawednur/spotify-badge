from io import BytesIO
import os
import json
import random
import requests

from colorthief import ColorThief
from base64 import b64encode, b64decode
from dotenv import load_dotenv, find_dotenv
from flask import Flask, Response, render_template, request
from markupsafe import escape

load_dotenv(find_dotenv())

# Spotify scopes:
#   user-read-currently-playing
#   user-read-recently-played
#   user-top-read (optional, for the top tracks panel)
PLACEHOLDER_IMAGE = ""
SPOTIFY_CLIENT_ID = os.getenv("SPOTIFY_CLIENT_ID")
SPOTIFY_SECRET_ID = os.getenv("SPOTIFY_SECRET_ID")
SPOTIFY_REFRESH_TOKEN = os.getenv("SPOTIFY_REFRESH_TOKEN")
SPOTIFY_TOKEN = ""

FALLBACK_THEME = "spotify.html.j2"
TEMPLATES_PATH = os.path.join(os.path.dirname(__file__), "templates.json")
REQUEST_TIMEOUT = 10

REFRESH_TOKEN_URL = "https://accounts.spotify.com/api/token"
NOW_PLAYING_URL = "https://api.spotify.com/v1/me/player/currently-playing"
RECENTLY_PLAYING_URL = (
    "https://api.spotify.com/v1/me/player/recently-played?limit=10"
)
# short_term is Spotify's finest granularity (roughly the last 4 weeks)
TOP_TRACKS_URL = (
    "https://api.spotify.com/v1/me/top/tracks?time_range=short_term&limit={count}"
)
MAX_TOP_TRACKS = 5

app = Flask(__name__)


def getAuth():
    return b64encode(f"{SPOTIFY_CLIENT_ID}:{SPOTIFY_SECRET_ID}".encode()).decode(
        "ascii"
    )


def refreshToken():
    data = {
        "grant_type": "refresh_token",
        "refresh_token": SPOTIFY_REFRESH_TOKEN,
    }

    headers = {"Authorization": "Basic {}".format(getAuth())}
    response = requests.post(
        REFRESH_TOKEN_URL, data=data, headers=headers,
        timeout=REQUEST_TIMEOUT).json()

    try:
        return response["access_token"]
    except KeyError:
        print(json.dumps(response))
        print("\n---\n")
        raise KeyError(str(response))


def get(url):
    global SPOTIFY_TOKEN

    if (SPOTIFY_TOKEN == ""):
        SPOTIFY_TOKEN = refreshToken()

    response = requests.get(
        url, headers={"Authorization": f"Bearer {SPOTIFY_TOKEN}"},
        timeout=REQUEST_TIMEOUT)

    if response.status_code == 401:
        SPOTIFY_TOKEN = refreshToken()
        response = requests.get(
            url, headers={"Authorization": f"Bearer {SPOTIFY_TOKEN}"},
            timeout=REQUEST_TIMEOUT)

    if response.status_code == 204:
        raise Exception(f"{url} returned no data.")

    response.raise_for_status()
    return response.json()


def barGen(barCount):
    barCSS = ""
    left = 1
    for i in range(1, barCount + 1):
        anim = random.randint(500, 1000)
        # below code generates random cubic-bezier values
        x1 = random.random()
        y1 = random.random()*2
        x2 = random.random()
        y2 = random.random()*2
        barCSS += (
            ".bar:nth-child({})  {{ left: {}px; animation-duration: 15s, {}ms; animation-timing-function: ease, cubic-bezier({},{},{},{}); }}".format(
                i, left, anim, x1, y1, x2, y2
            )
        )
        left += 4
    return barCSS


def gradientGen(imageBytes, color_count):
    colortheif = ColorThief(BytesIO(imageBytes))
    palette = colortheif.get_palette(color_count)
    return palette


def getTemplate():
    try:
        with open(TEMPLATES_PATH, "r") as file:
            templates = json.loads(file.read())
        return templates["templates"][templates["current-theme"]]
    except Exception as e:
        print(f"Failed to load templates.\r\n```{e}```")
        return FALLBACK_THEME


def getTopTracks(count):
    if count <= 0:
        return []
    try:
        data = get(TOP_TRACKS_URL.format(count=count))
    except Exception as e:
        # most likely a refresh token without the user-top-read scope;
        # hide the panel instead of breaking the whole badge
        print(f"Top tracks unavailable: {e}")
        return []

    tracks = []
    for entry in data.get("items", []):
        artists = entry.get("artists") or [{"name": ""}]
        tracks.append({
            "name": str(escape(entry.get("name", ""))),
            "artist": str(escape(artists[0].get("name", ""))),
            "url": entry.get("external_urls", {}).get("spotify", "#"),
        })
    return tracks


def makeSVG(data, background_color, border_color, topTracks):
    barCount = 84
    contentBar = "".join(["<div class='bar'></div>" for _ in range(barCount)])
    barCSS = barGen(barCount)

    # data["item"] is null for podcasts, ads and local files, so fall back
    # to recently played in those cases too
    if data and "is_playing" in data and data.get("item"):
        item = data["item"]
        currentStatus = "Vibing to:"
    else:
        #contentBar = "" #Shows/Hides the EQ bar if no song is currently playing
        currentStatus = "Recently played:"
        recentPlays = get(RECENTLY_PLAYING_URL)
        items = [
            play["track"] for play in recentPlays.get("items", [])
            if play.get("track")
        ]
        if not items:
            raise Exception("No recently played tracks available.")
        item = random.choice(items)

    images = item["album"]["images"]
    if not images:
        imageBytes = b64decode(PLACEHOLDER_IMAGE)
    else:
        # index 1 is the medium (300px) size when available
        artURL = images[1]["url"] if len(images) > 1 else images[0]["url"]
        imageBytes = requests.get(artURL, timeout=REQUEST_TIMEOUT).content
    image = b64encode(imageBytes).decode("ascii")
    barPalette = gradientGen(imageBytes, 4)
    songPalette = gradientGen(imageBytes, 2)

    artistName = str(escape(item["artists"][0]["name"]))
    songName = str(escape(item["name"]))
    songURI = item["external_urls"]["spotify"]
    artistURI = item["artists"][0]["external_urls"]["spotify"]

    dataDict = {
        "contentBar": contentBar,
        "barCSS": barCSS,
        "artistName": artistName,
        "songName": songName,
        "songURI": songURI,
        "artistURI": artistURI,
        "image": image,
        "status": currentStatus,
        "background_color": background_color,
        "border_color": border_color,
        "barPalette": barPalette,
        "songPalette": songPalette,
        "topTracks": topTracks,
        "cardWidth": 800 if topTracks else 480,
        "cardHeight": 173
    }

    return render_template(getTemplate(), **dataDict)


def toHexColor(value, fallback):
    value = (value or "").strip().lstrip("#")
    if len(value) in (3, 4, 6, 8) and all(c in "0123456789abcdefABCDEF" for c in value):
        return value
    return fallback


@app.route("/", defaults={"path": ""})
@app.route("/<path:path>")
@app.route('/with_parameters')
def catch_all(path):
    background_color = toHexColor(request.args.get('background_color'), "181414")
    border_color = toHexColor(request.args.get('border_color'), "181414")

    try:
        topCount = int(request.args.get("top_tracks", MAX_TOP_TRACKS))
    except ValueError:
        topCount = MAX_TOP_TRACKS
    topTracks = getTopTracks(max(0, min(topCount, MAX_TOP_TRACKS)))

    try:
        data = get(NOW_PLAYING_URL)
    except Exception:
        data = None

    svg = makeSVG(data, background_color, border_color, topTracks)

    resp = Response(svg, mimetype="image/svg+xml")
    resp.headers["Cache-Control"] = "s-maxage=1"

    return resp


if __name__ == "__main__":
    app.run(host="0.0.0.0", debug=True, port=os.getenv("PORT") or 5000)
